...
- **Firebase Auth** on the SPA (`src/lib/firebase.js`, `FirebaseAuthProvider`).
- Chat UI talks only to jerry-api via `src/api/chat.js` + `useChat`.
- AuthProvider syncs Mongo via `POST /api/auth/sync` with a Firebase ID token.
...
- Production CORS/API URL documented in `PRODUCTION.md`; `vercel.json` SPA rewrites.
